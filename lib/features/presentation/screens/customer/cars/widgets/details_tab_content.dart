...
class DetailsTabContent extends StatelessWidget {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // Discounts
          if (car.discounts.isNotEmpty) ...[
            sectionTitleBuilder('Available Discounts'),
            const SizedBox(height: 12),
            _buildDiscounts(),
            const SizedBox(height: 16),
          ],
  ...
  Widget _buildSpecifications(BuildContext context) {
    return Row(
      children: [
        SpecItem(
          svgAssetPath: 'assets/svg/seats.svg',
          text: car.seatsCount,
        ),
        SpecItem(
        ...
        SpecItem(
          svgAssetPath: car.transmissionType.toLowerCase().contains('auto')
    ...
    return Column(
      children: [
        RentalDetailsItem(
          title: 'Availability',
          value:
              'From ${formatDate(car.availableFrom)} to ${formatDate(car.availableTo)}',
        ),
        RentalDetailsItem(
  ...
  Widget _buildInclusions(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Row(
        children: car.inclusions.map((inclusion) {
          return Padding(
  ...
  Widget _buildExtraCharges(BuildContext context) {
    return Column(
      children: car.extraCharges.entries.map((entry) {
  ...
  Widget _buildDiscounts() {
    return Column(
      children: car.discounts.entries.map((entry) {
        return DiscountItem(
          title: entry.key,
          discount: '${entry.value.toStringAsFixed(0)}%',
        );
      }).toList(),
    );
  }
  ...
  Widget _buildRentalRequirements() {
    return Column(
      children: car.rentalRequirements.entries.map((entry) {
